...
The camera mode uses a value from the `CameraModeValue` enum. The following values are possible:
...
> Tip: To change the player's camera mode, use a [Camera modifier area](/creator/development-guide/avatar-modifiers/#camera-modifiers).
