...
> Note: The rotation of the `PlayerEntity` is updated at a throttled rate of 10 times per second. Due to this, positions & rotations may lag slightly in relation to the scene that runs at 30 FPS under ideal conditions.

TODO: check if that's still true
...
TODO: how to get player camera mode???
...
> Tip: To change the player's camera, use a [Camera modifier area](/creator/development-guide/avatar-modifiers/#camera-modifiers).
