...
> Note: You must add a _System_ to the engine before its functions start being called.
...
Most of the time, you don't want a system's function to iterate over the entire set of entities in the scene, as this could be very costly in terms of processing power. To avoid this, you can [query components](/creator/development-guide/querying-components), to only iterate over the relevant entities.
...
For more complex use cases, where there may be multiple delays and loops being created dynamically, it may be worth defining a custom component to store an individual timer value for each entity. See [Custom components](/creator/development-guide/custom-components).
